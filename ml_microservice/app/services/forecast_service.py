#!/usr/bin/env python3
"""
AI Market Forecasting Service
Adapted from python_scripts/forecast.py for integration into ml_microservice
"""
...
def perform_forecast(forecast_data: List[Dict[str, Any]], periods: int) -> Dict[str, Any]:
    """
    Perform time-series forecasting using multiple fallback methods
    """
    logger.info("=== PYTHON FORECAST SERVICE START ===")
    logger.info(f"Performing forecast for {periods} periods")
    ...
    if forecast_data and len(forecast_data) > 0:
        try:
            ...
            print("Using Prophet for forecasting", file=__import__("sys").stderr)

            forecast_success = _perform_prophet_forecast(forecast_data, periods, forecast_result)

        except Exception as import_error:
            print(f"Prophet failed, using fallback: {import_error}", file=__import__("sys").stderr)

        if not forecast_success:
            forecast_result = _perform_simple_forecast(forecast_data, periods)
...
def _perform_prophet_forecast(forecast_data: List[Dict[str, Any]], periods: int, forecast_result: Dict[str, Any]) -> bool:
...
def _perform_simple_forecast(forecast_data: List[Dict[str, Any]], periods: int) -> Dict[str, Any]:
