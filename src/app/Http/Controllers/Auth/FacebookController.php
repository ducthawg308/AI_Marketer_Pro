...
class FacebookController extends Controller
{
    ...
    public function handleFacebookCallback()
    {
        try {
            ...
            // Log kết quả fetch pages
            Log::info('Pages fetched for user', [
                'user_id' => $user->id,
            ...
            if (empty($pages)) {
                Log::warning('No pages found for user', [
                    'user_id' => $user->id,
                    'token_preview' => substr($longToken, 0, 20) . '...'
                ]);
                session()->flash('warning', 'Không tìm thấy Page nào. Đảm bảo tài khoản của bạn là Admin của ít nhất 1 Page.');
            }
    ...
    /**
     * Debug endpoint để kiểm tra kết nối Facebook
     */
    public function debugFacebookConnection()
    {
        $user = Auth::user();
        ...
        try {
            ...
            $debug = Http::get("https://graph.facebook.com/" . self::GRAPH_VERSION . "/debug_token", [
                'input_token' => $token,
                'access_token' => $appToken
            ])->json();
            
            // Get user info
            $me = Http::withToken($token)->get("https://graph.facebook.com/" . self::GRAPH_VERSION . "/me", [
                'fields' => 'id,name,email',
                'appsecret_proof' => $this->appSecretProof($token)
            ])->json();
            
            // Get permissions
            $permissions = Http::withToken($token)->get(
                "https://graph.facebook.com/" . self::GRAPH_VERSION . "/me/permissions",
                ['appsecret_proof' => $this->appSecretProof($token)]
            )->json();
            
            // Get accounts
            $accounts = Http::withToken($token)->get(
            ...
            return response()->json([
                ...
                'accounts' => $accounts,
                'user_pages_in_db' => UserPage::where('user_id', $user->id)->get()
            ]);
        } catch (\Throwable $e) {
            return response()->json([
                'error' => $e->getMessage(),
    ...
    private function fetchAllPages(string $userToken): array
    {
        ...
        $params = [
            'fields'          => 'id,name,access_token,category,about,link,picture,fan_count,is_published,tasks',
            'limit'           => 100,
            'appsecret_proof' => $this->appSecretProof($userToken),
        ];
        ...
        try {
            do {
                $resp = Http::withToken($userToken)->get($url, $params);
                ...
                if (isset($data['error'])) {
                    Log::error('Facebook API error when fetching pages', [
                        'error' => $data['error'],
                ...
                if (!empty($data['data'])) {
                    ...
                    Log::info('Pages data found', ['count' => count($data['data'])]);
                } else {
                    Log::warning('No pages data returned from Facebook', [
                        'response' => $data
                    ]);
                }
            ...
            Log::info('Total pages fetched: ' . count($all), [
