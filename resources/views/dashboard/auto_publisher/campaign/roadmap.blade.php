<x-app-dashboard>
  <div class="container mx-auto px-8 py-8">
      ...
      <div class="p-8">
        <div class="flex items-center justify-between">
          <div class="flex items-center space-x-6">
            <div class="relative">
              <div
                class="w-16 h-16 bg-gradient-to-r from-purple-500 to-purple-600 rounded-2xl flex items-center justify-center shadow-xl">
    ...
    <div class="grid grid-cols-2 gap-6 mb-8 lg:grid-cols-4">
      <div class="bg-white rounded-2xl shadow-lg border border-gray-200 p-6 text-center">
        <div
          ...
          <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
              d="M19 11H5m14-4V3a2 2 0 00-2-2H7a2 2 0 00-2 2v4a2 2 0 002 2h10a2 2 0 002-2z"></path>
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
      ...
      <div class="bg-white rounded-2xl shadow-lg border border-gray-200 p-6 text-center">
        <div
          class="w-12 h-12 bg-gradient-to-r from-orange-500 to-orange-600 rounded-xl flex items-center justify-center mx-auto mb-4">
          <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
              d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z">
            </path>
          ...
          <div class="lg:col-span-1">
            <div class="bg-gray-50 rounded-2xl border border-gray-200 p-6">
              <div class="flex items-center space-x-3 mb-4">
                <div class="w-8 h-8 bg-blue-100 rounded-lg flex items-center justify-center">
                  <svg class="w-4 h-4 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                      d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2H9a2 2 0 01-2-2v-2a2 2 0 012-2z">
                    </path>
                  </svg>
                </div>
              ...
              <div id="ads-list" class="space-y-4 max-h-96 overflow-y-auto">
                @foreach($ads as $ad)
                  <div
                    class="ad-item bg-white p-4 border border-gray-200 rounded-xl cursor-move hover:border-primary-300 hover:shadow-md transition-all duration-200"
                    data-ad-id="{{ $ad->id }}">
                    <div class="flex items-start space-x-3">
                      ...
                      <div class="flex-1 min-w-0">
                        ...
                        <span
                          class="inline-block px-2 py-1 text-xs rounded-full mt-2 font-medium
            ...
            <div class="bg-gray-50 rounded-2xl border border-gray-200 p-6">
              ...
              <div class="space-y-6 max-h-96 overflow-y-auto" id="roadmap">
                @foreach($roadmap as $day)
                  <div
                    class="bg-white border border-gray-200 rounded-2xl overflow-hidden hover:shadow-md transition-shadow duration-200">
                    <div
                      class="flex items-center justify-between p-6 border-b border-gray-200 bg-gradient-to-r from-gray-50 to-white">
                      <div class="flex items-center space-x-4">
                        <div
                          class="w-10 h-10 bg-gradient-to-r from-blue-500 to-purple-600 rounded-lg flex items-center justify-center shadow-sm">
                    ...
                    <div class="p-6">
                      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-{{ min(4, $day['post_count']) }}">
                        @foreach($day['posts'] as $post)
                          <div
                            class="slot-item bg-gray-50 border-2 border-dashed border-gray-300 rounded-xl p-4 flex items-center justify-between min-h-[80px] hover:bg-gray-100 hover:border-primary-300 transition-all duration-200 cursor-pointer group"
                            data-slot-id="{{ $post['slot_id'] }}" data-date="{{ $day['date']->format('Y-m-d') }}"
                            data-time="{{ $post['suggested_time'] }}" data-page-index="{{ $post['page_index'] }}">
                            <div class="flex-1">
                              <div class="mb-2">
                                <input type="time"
                                  class="slot-time-input px-2 py-1 bg-white border border-gray-300 rounded-md focus:ring-primary-500 focus:border-primary-500 text-sm font-semibold text-gray-900 w-[90px] cursor-pointer"
                                  value="{{ $post['suggested_time'] }}" min="{{ $campaign->default_time_start }}"
                                  max="{{ $campaign->default_time_end }}" onclick="event.stopPropagation()">
                              </div>
                            ...
                            <button
                              class="remove-ad text-red-600 hover:text-red-800 p-2 rounded-lg hover:bg-red-50 transition-colors duration-200 opacity-0 group-hover:opacity-100"
      ...
      <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden">
        ...
        <div class="p-6">
          ...
          <div class="flex justify-end space-x-4">
            ...
            <button type="button" onclick="executeConfirmedAction()"
              class="px-6 py-3 bg-orange-600 text-white rounded-xl hover:bg-orange-700 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 hover:scale-105 flex items-center">
              <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md">
        ...
        <div class="p-6 text-center">
          ...
          <button type="button" onclick="closeSuccessModal()"
            class="px-6 py-3 bg-green-600 text-white rounded-xl hover:bg-green-700 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 hover:scale-105">
            OK
          </button>
        </div>
      </div>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function () {
      ...
      function assignAdToSlot(slotId, adId) {
        ...
        const currentTime = timeInput ? timeInput.value : slot.dataset.time;
        ...
        timeHeader.className = 'flex justify-between items-center w-full mb-2 border-b border-green-100 pb-2';
        timeHeader.innerHTML = `
                    <input type="time" class="slot-time-input px-1 py-1 bg-green-50 border border-green-200 rounded-md focus:ring-green-500 focus:border-green-500 text-xs font-semibold text-green-800 w-[95px] cursor-pointer" value="${currentTime}" min="{{ $campaign->default_time_start }}" max="{{ $campaign->default_time_end }}" onclick="event.stopPropagation()">
                    <span class="text-xs text-green-700 font-medium px-2 py-1 bg-green-100 rounded-md">Page ${parseInt(slot.dataset.pageIndex) + 1}</span>
                `;
      ...
      function removeAdFromSlot(slotId) {
        ...
        const originalContent = `
                    <div class="flex-1">
                        <div class="mb-2">
                            <input type="time" class="slot-time-input px-2 py-1 bg-white border border-gray-300 rounded-md focus:ring-primary-500 focus:border-primary-500 text-sm font-semibold text-gray-900 w-[90px] cursor-pointer" value="${currentTime}" min="{{ $campaign->default_time_start }}" max="{{ $campaign->default_time_end }}" onclick="event.stopPropagation()">
                        </div>
                        <div class="text-sm text-gray-600">
                            <svg class="w-4 h-4 inline mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      function saveAssignments(action) {
        ...
        const slotsData = Object.entries(slotAssignments).map(([slotId, adId]) => {
          const slot = document.querySelector(`[data-slot-id="${slotId}"]`);
          const timeInput = slot.querySelector('.slot-time-input');
          const timeValue = timeInput ? timeInput.value : slot.dataset.time;
          return {
