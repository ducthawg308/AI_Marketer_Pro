    ...
    <div class="min-h-screen bg-primary-50 p-6">
        <div class="max-w-4xl mx-auto bg-white shadow-lg rounded-xl overflow-hidden border border-gray-100">
            <div class="p-6">
                ...
                <form action="{{ route('dashboard.audienceconfig.update', [$item->id]) }}" method="POST" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <select id="industry" name="industry" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-primary-600 focus:border-primary-600 sm:text-sm" required>
                            ...
                            <option value="fashion" {{ old('industry', $item->industry) === 'fashion' ? 'selected' : '' }}>Thời trang</option>
                            <option value="technology" {{ old('industry', $item->industry) === 'technology' ? 'selected' : '' }}>Công nghệ</option>
                            <option value="food" {{ old('industry', $item->industry) === 'food' ? 'selected' : '' }}>Thực phẩm</option>
                            <option value="other" {{ old('industry', $item->industry) === 'other' ? 'selected' : '' }}>Khác</option>
                        </select>
