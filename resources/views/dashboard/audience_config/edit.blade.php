            ...
            <div class="p-6">
                ...
                <form action="{{ route('dashboard.audience_config.update', [$item->id]) }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        @php
                            $competitors = old('competitors', $item->competitors ?? []);
                            if (!is_array($competitors)) $competitors = [];
                            
                            // Ensure at least one empty item for UI if needed
                        ...
                        <div id="competitors-container" class="space-y-4">
                            ...
                            <div class="competitor-item border border-gray-200 rounded-lg p-4 bg-gray-50 relative {{ $index > 0 ? 'mt-4' : '' }}">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 pr-6">
                                    <div>
                                        ...
                                        <input type="text" name="competitors[{{ $index }}][name]" value="{{ $competitor['name'] ?? '' }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-primary-600 focus:border-primary-600 sm:text-sm" placeholder="Ví dụ: Shopee">
                                    </div>
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700">URL đối thủ</label>
                                        <input type="url" name="competitors[{{ $index }}][url]" value="{{ $competitor['url'] ?? '' }}" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:ring-primary-600 focus:border-primary-600 sm:text-sm" placeholder="https://">
                                    </div>
                                </div>
                            </div>
                            @endforeach
                    ...
                    <script>
                        document.addEventListener('DOMContentLoaded', function() {
                            ...
                            let competitorCount = {{ count($competitors) }};
